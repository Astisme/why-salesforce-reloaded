...
function listenToFileUpload() {
	...
	function readFile(file) {
		if (file.type !== "application/json") {
            postMessage({what: "error", message: "Invalid file type. Only JSON files are supported."});
			return;
		}
		...
		reader.onload = function (e) {
            try {
                ...
                if (Array.isArray(imported) && imported.every(item =>
                    ...
                    postMessage({what: "error", message: "Invalid JSON structure. Your file must contain an array in which each item must have 'tabTitle' and 'url' as strings."});
                }
            } catch (error) {
                postMessage({what: "error", message: `Error parsing JSON: ${error.message}`});
            }
		};
